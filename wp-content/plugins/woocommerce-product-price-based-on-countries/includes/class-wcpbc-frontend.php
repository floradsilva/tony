...
class WCPBC_Frontend {
	...
	public static function init_session() {
		...
		if ( is_user_logged_in() && version_compare( WC_VERSION, '3.0', '>=' ) ) {
			WC()->customer->save();
		}
	...
	public static function test_store_message() {
		...
		if ( $test_country && ! empty( WC()->countries->countries[ $test_country ] ) ) {
			...
			echo wp_kses_post( '<p class="demo_store">' . sprintf( __( '%1$sPrice Based Country%2$s test mode enabled for testing %3$s. You should do tests on private browsing mode. Browse in private with %4$sFirefox%7$s, %5$sChrome%7$s and %6$sSafari%7$s', 'wc-price-based-country' ), '<strong>', '</strong>', $country, '<a style="display:inline;float:none;text-decoration:underline;" target="_blank" href="https://support.mozilla.org/en-US/kb/private-browsing-use-firefox-without-history">', '<a style="display:inline;float:none;text-decoration:underline;" target="_blank" href="https://support.google.com/chrome/answer/95464?hl=en">', '<a style="display:inline;float:none;text-decoration:underline;" target="_blank" href="https://support.apple.com/kb/ph19216?locale=en_US">', '</a>' ) . '</p>' );
		}
	}
