...
<table class="shop_table shop_table_responsive my_account_orders woocommerce-orders-table woocommerce-MyAccount-orders woocommerce-orders-table--orders">
	...
	<thead>
		<tr>
			...
			<th class="order-total woocommerce-orders-table__header woocommerce-orders-table__header-order-total"><span class="nobr"><?php echo esc_html_x( 'Type', 'table heading', 'wdm-subscription-split-payment' ); ?></span></th>
			<th class="order-type woocommerce-orders-table__header woocommerce-orders-table__header-order-type"><span class="nobr"><?php echo esc_html_x( 'Total', 'table heading', 'woocommerce-subscriptions' ); ?></span></th>
	...
	<tbody>
		<?php foreach ( $subscription_orders as $subscription_order ) :
			...
			?><tr class="order woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $order->get_status() ); ?>">
				...
				<td class="order-type woocommerce-orders-table__cell woocommerce-orders-table__cell-order-type" data-title="<?php echo esc_attr_x( 'Type', 'Used in data attribute. Escaped', 'woocommerce-subscriptions' ); ?>">
					<?php
						$order_type = wcs_get_objects_property( $order, 'wssp_order_type' );

						if ( 'payment' === $order_type ) {
							echo esc_html( 'Payment', 'wdm-subscription-split-payment' );
						} elseif ( 'installment' === $order_type ) {
							echo esc_html( 'Installment', 'wdm-subscription-split-payment' );
						}	
					?>
				</td>
