	...
	public static function show_cart_item_subscription_options( $price, $cart_item, $cart_item_key ) {
		...
		if ( false === $force_subscription ) {
			...
			if ( $price_filter_exists ) {
				...
				$description = __( 'only now', 'woocommerce-all-products-for-subscriptions' );
			}
	...
	public static function show_cart_subscription_options() {
		...
		if ( $subscription_schemes = WCS_ATT_Cart::get_cart_subscription_schemes( 'cart-display' ) ) {
			...
			$options[] = array(
				'class'       => 'one-time-option',
				'value'       => '0',
				'description' => _x( 'No thanks.', 'cart subscription selection - negative response', 'woocommerce-all-products-for-subscriptions' ),
			...
			foreach ( $subscription_schemes as $subscription_scheme ) {
				...
				if ( $discount = $subscription_scheme->get_discount() ) {
					$discount_html = '<span class="wcsatt-sub-discount">' . sprintf( __( ' <span class="wcsatt-dash">&mdash;</span> and save %s&#37;', 'woocommerce-all-products-for-subscriptions' ), $discount ) . '</span>';
					$option_html = sprintf( _x( '%1$s%2$s', 'price html cart sub option with suffix', 'woocommerce-all-products-for-subscriptions' ), $option_html, $discount_html );
				}

				$description = sprintf( _x( 'Yes, %s.', 'cart subscription selection - positive response', 'woocommerce-all-products-for-subscriptions' ), $option_html );
