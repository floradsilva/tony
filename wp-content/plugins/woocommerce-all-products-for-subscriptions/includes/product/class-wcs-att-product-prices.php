			...
			// Subscription state is undefined? Construct a special price string.
				...
				if ( WCS_ATT_Product_Schemes::has_forced_subscription_scheme( $product ) ) {
					...
					if ( ( $has_variable_price || sizeof( $schemes ) > 1 ) && false === strpos( $price_html, $html_from_text ) ) {
						$price_html = sprintf( _x( '%1$s%2$s', 'Price range: from', 'woocommerce-all-products-for-subscriptions' ), $html_from_text, $price_html );
					}
					...
					if ( $price_filter_exists && apply_filters( 'wcsatt_price_html_discount_format', $use_discount_html_format, $product ) ) {
						...
						$discount_html     = '</small> <span class="wcsatt-sub-discount">' . sprintf( __( '%s&#37; off', 'woocommerce-all-products-for-subscriptions' ), $discount ) . '</span><small>';
						$suffix_price_html = sprintf( __( 'subscribe and get %1$s%2$s', 'woocommerce-all-products-for-subscriptions' ), $has_variable_discount ? __( ' up to', 'woocommerce-all-products-for-subscriptions' ) : '', $discount_html );
						...
						if ( sizeof( $schemes ) > 1 ) {
							$suffix_price_html = sprintf( _x( '%1$s%2$s', 'Price range: starting at', 'woocommerce-all-products-for-subscriptions' ), _x( '<span class="from">starting at </span>', 'subscriptions "starting at" price string', 'woocommerce-all-products-for-subscriptions' ), str_replace( $html_from_text, '', $base_scheme_price_html ) );
						} elseif ( $has_variable_price ) {
							$suffix_price_html = sprintf( _x( '%1$s%2$s', 'Price range: from', 'woocommerce-all-products-for-subscriptions' ), _x( '<span class="from">from </span>', 'subscription "from" price string', 'woocommerce-all-products-for-subscriptions' ), str_replace( $html_from_text, '', $base_scheme_price_html ) );
						} else {
							$suffix_price_html = $base_scheme_price_html;
						}

						if ( WCS_ATT_Product_Schemes::price_filter_exists( $schemes ) ) {
							$suffix = '<small class="wcsatt-sub-options">' . sprintf( _n( ' <span class="wcsatt-dash">&mdash;</span> or %s', ' <span class="wcsatt-dash">&mdash;</span> subscription plans %s', sizeof( $schemes ), 'woocommerce-all-products-for-subscriptions' ), $suffix_price_html ) . '</small>';
						} else {
							$suffix = '<small class="wcsatt-sub-options">' . sprintf( _n( ' <span class="wcsatt-dash">&mdash;</span> subscription plan available', ' <span class="wcsatt-dash">&mdash;</span> subscription plans available', sizeof( $schemes ), 'woocommerce-all-products-for-subscriptions' ), $suffix_price_html ) . '</small>';
						}
					}

					$price_html = sprintf( _x( '%1$s%2$s', 'price html sub options suffix', 'woocommerce-all-products-for-subscriptions' ), $price_html, $suffix );
				}
			}
		}
