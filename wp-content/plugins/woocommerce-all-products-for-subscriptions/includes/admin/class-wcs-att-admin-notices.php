...
class WCS_ATT_Admin_Notices {
	...
	public static function welcome_notice() {
		...
		<p class="sw-welcome-text"></span><?php esc_attr_e( 'Thank you for installing WooCommerce All Products For Subscriptions.', 'woocommerce-all-products-for-subscriptions' ); ?><br/><?php esc_attr_e( 'Let\'s start by adding some subscription plans to an existing product.', 'woocommerce-all-products-for-subscriptions' ); ?></p>
		<div class="sw-expanding-button-container">
			<div class="sw-expanding-button sw-expanding-button--large">
				...
				<select class="wc-product-search" id="satt_product_search" name="satt_product" data-placeholder="<?php _e( 'Search for a product&hellip;', 'woocommerce-all-products-for-subscriptions' ); ?>" data-action="woocommerce_json_search_satt_onboarding" multiple="multiple" data-limit="200">
					<option></option>
				</select>
			</div><?php
		?></div><?php
	...
	public static function add_cart_plans_onboarding_notice() {
		...
		<p class="onboarding-details"><?php echo sprintf( __( 'Did you know that you can also use <strong>All Products For Subscriptions</strong> to offer subscription options on the cart page?</br>For details, check out the <a href="%1$s">documentation</a>. Then, configure cart subscription plans <a href="%2$s">here</a>.', 'woocommerce-all-products-for-subscriptions' ), WCS_ATT::DOCS_URL, $settings_link ); ?></p>
		<?php
