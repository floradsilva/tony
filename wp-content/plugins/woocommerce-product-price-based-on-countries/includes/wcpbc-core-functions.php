...
function wcpbc_geoipdb_exists() {
	...
	if ( ! $exists && is_callable( array( 'WC_Geolocation', 'get_local_database_path' ) ) ) {
...
function wcpbc_update_product_pricing( $post_id, $zone, $data = array() ) {
	...
	if ( empty( $data['_price_method'] ) || wcpbc_is_exchange_rate( $data['_price_method'] ) ) {
		...
		} elseif ( ! wcpbc_empty_nozero( $data['_sale_price'] ) && $data['_sale_price_dates_from'] && $data['_sale_price_dates_from'] <= strtotime( 'NOW', current_time( 'timestamp' ) ) ) {
			$data['_price'] = $data['_sale_price'];
		} else {
