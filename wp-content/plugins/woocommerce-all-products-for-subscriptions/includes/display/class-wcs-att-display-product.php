...
class WCS_ATT_Display_Product {
	...
	public static function get_subscription_options_content( $product, $parent_product = null ) {
		...
		if ( false === $force_subscription ) {
			...
			$none_string = _x( 'none', 'product subscription selection - negative response', 'woocommerce-all-products-for-subscriptions' );
		...
		foreach ( $subscription_schemes as $subscription_scheme ) {
			...
			// See https://github.com/Prospress/woocommerce-subscriptions/commit/2175da54e21b996eca3c84111d524a63efd8a275#diff-2da12a954db622d484655355c763282c
			if ( version_compare( WC_Subscriptions::$version, '2.2.19', '<' ) ) {
			...
			$description = false === $force_subscription ? sprintf( _x( '%s', 'product subscription selection - positive response', 'woocommerce-all-products-for-subscriptions' ), $sub_price_html ) : $sub_price_html;
			...
			$options[] = array(
				...
				'data'        => apply_filters( 'wcsatt_single_product_subscription_option_data', $option_data, $subscription_scheme, $product, $parent_product )
			);
		}
